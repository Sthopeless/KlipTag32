...
void RC522::loop() {
  ...
  switch (state_) {
    ...
    case STATE_READ_SERIAL_DONE: {
      ...
      if (this->read_ndef_text_ultralight_(ndef_text)) {
        for (auto *trigger : this->triggers_on_ndef_text_) {
          trigger->process(ndef_text);
        }
      }
...
RC522::StatusCode RC522::await_transceive_() {
  ...
  this->last_rx_valid_bits_ = valid_bits_local;
  ...
  // Tell about collisions
  if (valid_bits_local) {
    if (valid_bits_local == 4 && back_length_ == 1) {
      const uint8_t ack = buffer_[send_len_] & 0x0F;
      if (ack == 0x0A) {
        return STATUS_OK;
      }
    }
    ESP_LOGW(TAG, "only %d valid bits received, tag distance to high? Error code is 0x%x", valid_bits_local,
             error_reg_value);  // TODO: is this always due to collisions?
    return STATUS_ERROR;
  }
...
bool RC522::transceive_read_page_(uint8_t page, uint8_t *out16) {
  buffer_[0] = PICC_CMD_MF_READ;
  buffer_[1] = page;
  pcd_calculate_crc_(buffer_, 2);

  StatusCode status = STATUS_WAITING;
  while ((status = await_crc_()) == STATUS_WAITING) {
    delay(1);
  }
  awaiting_comm_ = false;
  if (status != STATUS_OK) {
  ...
  while ((status = await_transceive_()) == STATUS_WAITING) {
    delay(1);
  }
  awaiting_comm_ = false;
  if (status != STATUS_OK || back_length_ < 16) {
    return false;
  }

  for (uint8_t i = 0; i < 16; i++) {
    out16[i] = buffer_[send_len_ + i];
  }
  return true;
}

bool RC522::transceive_ultralight_write_page_(uint8_t page, const uint8_t *data4) {
  buffer_[0] = PICC_CMD_UL_WRITE;
  buffer_[1] = page;
  buffer_[2] = data4[0];
  buffer_[3] = data4[1];
  buffer_[4] = data4[2];
  buffer_[5] = data4[3];
  pcd_calculate_crc_(buffer_, 6);
  ...
  if (!(back_length_ == 1 && this->last_rx_valid_bits_ == 4)) {
    return false;
  }
  const uint8_t ack = buffer_[send_len_] & 0x0F;
  return ack == 0x0A;
...
bool RC522::parse_ndef_text_record_(const std::vector<uint8_t> &bytes, std::string &text) {
  text.clear();
  if (bytes.size() < 5) {
    return false;
  }

  // Walk the full TLV stream and parse the first NDEF Message TLV (0x03).
  size_t i = 0;
  while (i < bytes.size()) {
    const uint8_t tlv_type = bytes[i++];
    if (tlv_type == 0x00) {
    ...
    if (bytes[i] == 0xFF) {
      ...
      tlv_len = (static_cast<size_t>(bytes[i + 1]) << 8) | bytes[i + 2];
      i += 3;
    } else {
      tlv_len = bytes[i++];
    }
    if (i + tlv_len > bytes.size()) {
      return false;
    }

    if (tlv_type != 0x03) {
      i += tlv_len;  // Skip non-NDEF TLVs
      continue;
    }

    // NDEF message starts at bytes[i], length tlv_len.
    if (tlv_len < 5) {
      return false;
    }
    size_t n = i;
    const uint8_t header = bytes[n++];
    if (n >= bytes.size()) {
      return false;
    }
    const uint8_t type_len = bytes[n++];
    const bool short_record = (header & 0x10) != 0;
    const bool has_id_len = (header & 0x08) != 0;
    ...
    if (short_record) {
      ...
      payload_len = (static_cast<size_t>(bytes[n]) << 24) | (static_cast<size_t>(bytes[n + 1]) << 16) |
                    (static_cast<size_t>(bytes[n + 2]) << 8) | bytes[n + 3];
      n += 4;
    }
    ...
    if (n + type_len + id_len + payload_len > i + tlv_len) {
      return false;
    }

    if (type_len != 1 || bytes[n] != 'T') {
      return false;
    }
    n += type_len;
    n += id_len;
    ...
    const uint8_t status_byte = bytes[n++];
    const size_t lang_len = status_byte & 0x3F;
    ...
    for (size_t j = 0; j < text_len; j++) {
      const char c = static_cast<char>(bytes[n + j]);
      if (c == '\0') {
        break;
      }
      text.push_back(c);
    ...
    while (!text.empty() &&
           (text.back() == '\r' || text.back() == '\n' || !std::isprint(static_cast<unsigned char>(text.back())))) {
      text.pop_back();
    }
    return !text.empty();
  }

  // Fallback: look for plain text payload markers in printable bytes.
  std::string printable;
  printable.reserve(bytes.size());
  for (uint8_t b : bytes) {
    if (b == '\n' || b == '\r' || (b >= 0x20 && b < 0x7F)) {
      printable.push_back(static_cast<char>(b));
    }
  }
  size_t s = printable.find("SPOOL:");
  if (s != std::string::npos) {
    size_t e = printable.find('\0', s);
    text = printable.substr(s, e == std::string::npos ? std::string::npos : e - s);
    return !text.empty();
  }
  return false;
}

bool RC522::read_ndef_text_ultralight_(std::string &text) {
  std::vector<uint8_t> raw;
  raw.reserve(192);
  uint8_t page_buf[16];

  for (uint8_t page = 4; page <= 36; page += 4) {
    if (!this->transceive_read_page_(page, page_buf)) {
      break;
    }
    for (uint8_t i = 0; i < 16; i++) {
      raw.push_back(page_buf[i]);
    }
  }
  ...
  if (!this->parse_ndef_text_record_(raw, text)) {
    ESP_LOGD(TAG, "NDEF text parse failed for UID '%s'", format_hex_pretty(this->current_uid_, '-', false).c_str());
    return false;
  }

  ESP_LOGI(TAG, "Read NDEF text: %s", text.c_str());
  return true;
}

bool RC522::write_ndef_text_ultralight(const std::string &text) {
  if (text.empty()) {
    ESP_LOGW(TAG, "Refusing to write empty NDEF text");
    return false;
  }

  std::vector<uint8_t> payload;
  payload.reserve(1 + 2 + text.size());
  payload.push_back(0x02);  // UTF-8, language code length 2
  payload.push_back('e');
  payload.push_back('n');
  payload.insert(payload.end(), text.begin(), text.end());
  if (payload.size() > 0xFF) {
    ESP_LOGW(TAG, "NDEF payload too long: %d", static_cast<int>(payload.size()));
    return false;
  }

  std::vector<uint8_t> ndef_record;
  ndef_record.reserve(5 + payload.size());
  ndef_record.push_back(0xD1);  // MB=1, ME=1, SR=1, TNF=Well-known
  ndef_record.push_back(0x01);  // Type length
  ndef_record.push_back(static_cast<uint8_t>(payload.size()));
  ndef_record.push_back('T');
  ndef_record.insert(ndef_record.end(), payload.begin(), payload.end());

  std::vector<uint8_t> tlv;
  tlv.reserve(ndef_record.size() + 6);
  tlv.push_back(0x03);  // NDEF Message TLV
  if (ndef_record.size() < 0xFF) {
    tlv.push_back(static_cast<uint8_t>(ndef_record.size()));
  } else {
    tlv.push_back(0xFF);
    tlv.push_back(static_cast<uint8_t>((ndef_record.size() >> 8) & 0xFF));
    tlv.push_back(static_cast<uint8_t>(ndef_record.size() & 0xFF));
  }
  tlv.insert(tlv.end(), ndef_record.begin(), ndef_record.end());
  tlv.push_back(0xFE);  // Terminator TLV

  const size_t max_user_bytes = 48;  // MIFARE Ultralight user area pages 4..15
  if (tlv.size() > max_user_bytes) {
    ESP_LOGW(TAG, "NDEF TLV too long for Ultralight (%d > %d)", static_cast<int>(tlv.size()), static_cast<int>(max_user_bytes));
    return false;
  }
  tlv.resize(max_user_bytes, 0x00);

  for (uint8_t page = 4; page <= 15; page++) {
    const size_t offset = (page - 4) * 4;
    if (!this->transceive_ultralight_write_page_(page, &tlv[offset])) {
      ESP_LOGW(TAG, "Ultralight page write failed at page %u", page);
      return false;
    }
  }

  ESP_LOGI(TAG, "Wrote NDEF text: %s", text.c_str());
  return true;
}
