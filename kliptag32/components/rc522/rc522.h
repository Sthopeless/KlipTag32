...
class RC522 : public PollingComponent {
 public:
  ...
  void register_on_ndef_text_trigger(RC522NdefTextTrigger *trig) { this->triggers_on_ndef_text_.push_back(trig); }
  bool write_ndef_text_ultralight(const std::string &text);
 ...
 protected:
  ...
  uint8_t buffer_[32];      ///< buffer for communication. Increased from 9 so Ultralight READ (16B + CRC) fits.
  ...
  std::vector<RC522NdefTextTrigger *> triggers_on_ndef_text_;
  std::vector<uint8_t> current_uid_;

  bool read_ndef_text_ultralight_(std::string &text);
  bool transceive_read_page_(uint8_t page, uint8_t *out16);
  bool transceive_ultralight_write_page_(uint8_t page, const uint8_t *data4);
  bool parse_ndef_text_record_(const std::vector<uint8_t> &bytes, std::string &text);

  enum RC522Error {
    NONE = 0,
    RESET_FAILED,
  } error_code_{NONE};
  uint8_t last_rx_valid_bits_{0};
};
...
class RC522NdefTextTrigger : public Trigger<std::string> {
 public:
  void process(const std::string &text) { this->trigger(text); }
};
